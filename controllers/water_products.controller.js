...
export const water_productsController = {
  ...
  update: async function (req, res, next) {
    try {
      ...
      if (founded_product.length === 0)
        return res
          .status(404)
          .send({ message: `${id} water product not found` });
      const updated_product = await water_productsModel.findByIdAndUpdate(
        id.req.body,
        { new: true },
      );
      return res.status(200).send({
  ...
  delete: async function (req, res, next) {
    try {
      ...
      const founded_product = await water_productsModel.findById({ _id: id });
      if (founded_product.length === 0)
        return res.status(404).send({ message: `${id} product not found` });
      const updated_product = await water_productsModel.findByIdAndUpdate(
        id,
        req.body,
        { new: true },
