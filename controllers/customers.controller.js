...
export const customerController={
    ...
    update:async function(req,res,next){
        try{
            ...
            if(foundedcustomer.length===0) return res.status(404).send({message:`${id} customer not found`})
            const updated_customer=await customerModel.findByIdAndUpdate(id,req.body,{new:true})
